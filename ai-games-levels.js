...
chat()`,
    hint: `试试问它"你叫什么"、"你是谁"，或者直接说声"谢谢"、"你好吗"，看它是不是都能答上来；再说一句"我叫小明"，看它记不记得住；也可以试试"我是谁"这种话，看它是不是不会误当成在报名字了。`,
    walkthrough: [
      ...
      { lines: [37, 41], note: `跟原来一样：逐个检查RULES里的每一个关键词，命中就随机挑一句回复；一个都没命中就返回默认回复。` },
      { lines: [44, 55], note: `抓名字的函数：先用正则找"我叫XX"或"我是XX"，抓出后面一小段——这次多了两道防呆检查：抓到的词如果是"谁"这类疑问代词，或者整句话是以"不"、"说"这些明显不是在报名字的字开头，就当作没抓到，返回None，不会误判。` },
      { lines: [58, 70], note: `教学逻辑不变：先问关键词，再问希望听到这个关键词时怎么回复——两个都填了才真正写进RULES字典，返回True告诉外面"这次真的学会了一条"。` },
      { lines: [73, 79], note: `定义整个对话流程，开场白提到"会记住名字"和"连续两次没听懂会请教"这两个新行为；user_name专门用来存你的名字，一开始是None（还不知道）。` },
      { lines: [80, 87], note: `每一轮先试着从你的话里抓名字——如果抓到了（通过了防呆检查）、而且之前还不知道你的名字，就记下来并回一句确认，然后直接跳到下一轮。` },
      { lines: [88, 94], note: `正常的关键词匹配流程：挑一句回复；如果已经知道你的名字、而且这次是没命中关键词的默认回复，就在前面加上称呼；打印出来，说"再见"就结束对话。` },
      { lines: [96, 99], note: `判断刚才这句是不是默认回复——如果是，说明这次一个关键词都没命中，连续没命中的计数+1；命中过规则就清零，重新计数。` },
      { lines: [101, 104], note: `连续没命中够2次，就触发"教学"环节；真的教成功了才把taught_count加1，不管教没教成功都把计数清零，避免刚教完马上又立刻重复弹出来。` },
      { lines: [106, 112], note: `对话结束，报告教会了几条规则；如果记住了名字，额外点明这是"真正存进变量的记忆"，跟"关键词匹配"是两种不同的能力——一个是临时查表，一个是一直带着用的状态。` },
      { lines: [115, 115], note: `真正开始运行整个对话，前面都只是定义。` },
    ],
  },
  {
    id: 3,
