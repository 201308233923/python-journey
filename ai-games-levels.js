...
print("真实的AI（比如神经网络）原理类似，只是有几十亿个这样的神经元一起工作。")`,
  ...
  },
  {
    ...
    explain: `
      ...
      （"那是黄色的吗？"……），猜完颜色再用同样的方式猜形状，最后综合颜色+形状给出答案。要是
      颜色、形状都问遍了还是拿不准，AI会老实说猜不出来，直接问你这个水果叫什么。</p>
      <p>训练数据依然只有2条（红色+圆形=苹果，黄色+长条形=香蕉），所以不管AI问多少轮问题，
      只要你想的水果不是这两种特征组合，它照样会露馅——这就是"问得多"和"学得够"是两码事。</p>
    `,
    code: `# AI水果分类器：这次AI会像真的在猜一样，一步步问——颜色不对就换一个颜色再猜，
# 形状不对就换一个形状再猜，最后要是还猜不出来，就直接问你这个水果叫什么。
# 训练数据依然很少，就算多问几轮，遇到没学过的组合还是会露馅。
...
# 训练数据：只学过这2种"颜色+形状"的组合，剩下的水果AI压根没见过
TRAINING_DATA = {
    ("红色", "圆形"): "苹果",
    ("黄色", "长条形"): "香蕉",
}

print("这个AI只学过2个例子：红色+圆形=苹果，黄色+长条形=香蕉。")
...
while True:
    ...
    # 第一步：猜颜色——一个猜错了就换下一个，不是一次性问"是什么颜色"
    ...
    if key in TRAINING_DATA:
        print(f"AI猜：{TRAINING_DATA[key]}！（这个颜色+形状的组合我训练时见过）")
        if ask_yes_no("AI猜对了吗？"):
            print("猜对了！")
            correct += 1
        else:
            print("猜错了——这个特征组合虽然AI训练时见过，但撞上了别的水果，光靠颜色+形状分不清。")
    else:
        print("颜色和形状我都猜了，但这个组合我训练时没见过，猜不出来了……")
...
print("就算多问几轮，AI也只对训练数据里学过的组合有把握。")`,
    hint: `AI会先猜颜色、再猜形状，猜错了会自动换一个继续问，不用你操心。想让AI"露馅"，就想一个训练数据里没有的组合，比如绿色的西瓜、紫色的葡萄。`,
    walkthrough: [
      { lines: [1, 3], note: `开头注释点明这一关跟上一版的区别：这次AI会像真的在猜一样一步步问，猜错了自动换方向，而不是固定问几个问题就完事。` },
      { lines: [6, 7], note: `小工具函数：把"是/否"的文字回答转换成True/False，方便后面直接用在if判断里。` },
      { lines: [10, 14], note: `训练数据：只学过"红色+圆形=苹果"和"黄色+长条形=香蕉"这2种组合——注意这次key是文字（颜色、形状），不是布尔值了。` },
      ...
      { lines: [43, 44], note: `把猜到的颜色和形状拼成一个组合，当作查训练数据用的key；不管接下来猜不猜得出来，这一轮都要算进"总共考了几次"里。` },
      { lines: [45, 51], note: `如果这个颜色+形状的组合训练时见过，就报出对应的水果——但还是要问你"猜对了吗"，因为可能撞上了别的长得一样的水果，颜色+形状分不清就是分不清。` },
      { lines: [52, 56], note: `如果这个组合训练时没见过：AI没有硬编一个答案糊弄你，而是老实承认猜不出来，直接问你这是什么水果——问到答案之后，也诚实说明"这不算真的学会了，只是这次你告诉了我"。` },
      { lines: [58, 60], note: `问你还要不要继续考它，回答"否"（或者其他任何不是"是"的内容）就跳出循环，结束游戏。` },
      { lines: [61, 61], note: `打印一个空行，让下一轮的"第X轮"标题前面留个间隔，不会紧贴着上一轮的内容。` },
      { lines: [63, 65], note: `游戏结束，算出这一局的最终成绩，再次点明规律：不管AI问多少轮问题、问得多细致，它也只对训练数据里学过的组合有把握——这就是为什么不能盲目相信AI的每一个答案，得自己核实。` },
    ],
  },
  {
    id: 6,
