...
print("真实的AI（比如神经网络）原理类似，只是有几十亿个这样的神经元一起工作。")`,
  ...
  },
  {
    ...
    explain: `
      <p>这个AI"水果分类器"只学过2个例子：红色+圆形+个头大=苹果，不红+不圆+个头大=香蕉。这次问
      3个问题（不止2个），你心里随便想一个真实水果（苹果、香蕉、草莓、西瓜、樱桃……什么都行），
      AI猜一个答案，然后你自己判断它猜对了没有，如实告诉它。</p>
...
# 训练数据：(是否红色, 是否圆形, 个头是否比乒乓球大) -> 水果种类
# 注意这里刻意只给了2条数据，AI能学到的东西非常有限
TRAINING_DATA = {
    (True, True, True): "苹果",     # 红色 + 圆形 + 个头大
    (False, False, True): "香蕉",   # 不红 + 不圆 + 个头大
}


def ai_guess(is_red, is_round, is_big):
    key = (is_red, is_round, is_big)
...
print("这个AI只学过2个例子：红色+圆形+个头大=苹果，不红+不圆+个头大=香蕉。")
print("你心里想一个真实的水果（随便什么水果都行），回答三个问题，看AI能不能猜对！\\n")
...
while True:
    ...
    is_big = ask_yes_no("个头比乒乓球大吗？")

    guess = ai_guess(is_red, is_round, is_big)
    print(f"AI猜：{guess}")
    ...
    if is_correct:
        ...
        print("真实的AI也是一样：特征问得再多，只要训练数据没覆盖到，它照样会出错。")
...
print("它只对训练数据里出现过（而且特征描述得够细）的情况有把握。")`,
    ...
    walkthrough: [
      ...
      { lines: [3, 8], note: `训练数据：字典的key是"(是否红色, 是否圆形, 个头是否比乒乓球大)"这样三个布尔值的组合，value是水果名字——问的特征变多了，但训练例子依然只有2条，剩下的水果它都没学过。` },
      { lines: [11, 16], note: `预测函数：如果这三个特征的组合在训练数据里见过，就直接查表返回对应答案；没见过就只能从学过的两种里随便挑一个，返回"苹果（瞎猜的）"。` },
      ...
      { lines: [32, 34], note: `问你心里想的水果的三个特征——注意AI从头到尾都不知道你想的具体是哪种水果，只看得到这三个特征。` },
      { lines: [36, 36], note: `把这三个特征交给AI，拿到它的猜测。` },
      { lines: [37, 37], note: `把AI的猜测打印出来。` },
      { lines: [39, 39], note: `关键的一步：不是程序自己判断对错，而是问你——因为只有你自己知道心里想的到底是什么水果，程序没有别的办法验证。` },
      { lines: [40, 40], note: `不管对错，这一轮都要算进"总共考了几次"里。` },
      { lines: [41, 43], note: `你说猜对了：记一分，打印祝贺。` },
      { lines: [44, 47], note: `你说猜错了：解释原因——要么这个特征组合AI压根没学过，要么撞上了别的水果（特征一样但答案不同），这就是训练数据不够/不够细致导致的错误。就算问的特征变多了，训练数据没覆盖到照样会错。` },
      { lines: [49, 51], note: `问你还要不要继续考它，回答"否"（或者其他任何不是"是"的内容）就跳出循环，结束游戏。` },
      { lines: [53, 55], note: `游戏结束，算出这一局的最终成绩，再次点明规律：AI只对训练数据里出现过、而且描述得够细的情况有把握，这就是为什么不能盲目相信AI的每一个答案，得自己核实。` },
    ],
  },
  {
    id: 6,
