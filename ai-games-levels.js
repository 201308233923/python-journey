...
if want_reverse == "是":
    ...
    hint: `先试试只说一个关键词，比如"恐龙"；再试试同时说两三个，比如"恐龙 化石 科普"，对比一下AI推荐的排序有没有变化。玩反过来那个模式时，AI报出第一个标签就试着猜猜看——越早猜中，说明你对"这个特征通常对应什么"的判断越准。`,
    walkthrough: [
      ...
      { lines: [18, 25], note: `搜索函数：数一下每本书命中了几个标签，把命中的标签也记下来，再按命中数量从高到低排序——命中越多的书排得越靠前。` },
      { lines: [28, 33], note: `新增的反向挑战函数：从书单里随机选一本当"秘密书"，把它的标签复制一份打乱顺序（避免每次都按书单里写的固定顺序透露），revealed用来记已经透露过的标签。` },
      { lines: [35, 36], note: `开场白：说明玩法——标签透露得越少就猜中，说明你越会抓关键信息。` },
      { lines: [38, 44], note: `每次从打乱的顺序里取一个新标签透露出来；同时算一下"目前书单里，标签全部符合已知条件的还剩几本"——如果刚好剩1本，提示你可以试着猜了。` },
      { lines: [46, 52], note: `读你的猜测——直接回车（不猜）就继续听下一个标签；猜对了把用了几个标签交出去；猜错了提示继续。` },
      { lines: [54, 55], note: `标签全透露完了你还没猜中，AI直接公布答案，同样把用了几个标签（也就是全部标签数）交出去。` },
      { lines: [58, 59], note: `开场白直接提示了这一关的技巧：关键词越具体，AI才越容易帮到你。` },
      { lines: [61, 62], note: `问你想看什么书，把你的回答交给搜索函数。` },
      { lines: [64, 68], note: `有结果：不只是列书名，还把每本书命中了几个关键词、具体是哪几个都亮出来，再点明"命中越多排得越靠前"。` },
      { lines: [69, 73], note: `如果一本都没搜到：AI坦白说没听懂，并且给出具体的改进建议——问得越模糊，AI越难猜中你要什么。` },
      { lines: [75, 80], note: `新增：问你要不要反过来玩，玩了就跑刚才那个挑战函数，根据用了几个标签给出总结（区分度越高的词，越少就能锁定答案），打印结算暗号——用的标签数不超过2个才算"赢"，页面看到这个暗号才会放庆祝的礼花。` },
    ],
  },
  {
    id: 7,
