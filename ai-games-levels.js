...
print("真实的AI（比如神经网络）原理类似，只是有几十亿个这样的神经元一起工作。")`,
  ...
  },
  {
    ...
    explain: `
      <p>这次AI会像真的在猜一样，一步步问：先猜颜色（"是红色吗？"），猜错了就换一个颜色接着猜
      （"那是黄色的吗？"……），猜完颜色再用同样的方式猜形状。但光凭颜色+形状还不够下结论——
      苹果、樱桃、荔枝都是"红色+圆形"，所以AI还会再多问一个大小，才最终给出答案；给出答案后
      还会问你"猜对了吗"，让你来判断。</p>
      <p>训练数据依然只有2条（红色+圆形+个头大=苹果，黄色+长条形+个头大=香蕉），所以不管
      AI问多少轮问题，只要你想的水果不是这个特征组合，它照样会露馅——这就是"问得多"和
      "学得够"是两码事。</p>
    `,
    code: `# AI水果分类器：这次AI会像真的在猜一样，一步步问——颜色不对就换一个颜色再猜，
# 形状不对就换一个形状再猜，光凭颜色+形状还分不清（比如苹果、樱桃、荔枝都是
# 红色+圆形），再多问一个大小，才下最终结论；结论出来后还要问你对不对。
...
# 训练数据：只学过这2种"颜色+形状+大小"的组合，剩下的水果AI压根没见过
TRAINING_DATA = {
    ("红色", "圆形", "大"): "苹果",
    ("黄色", "长条形", "大"): "香蕉",
}

print("这个AI只学过2个例子：红色+圆形+个头大=苹果，黄色+长条形+个头大=香蕉。")
print("你心里想一个真实的水果，AI会一步步猜，猜错了会换个方向继续猜！\\n")
...
while True:
    ...
    # 第一步：猜颜色——一个猜错了就换下一个
    if ask_yes_no("这个水果是红色的吗？"):
        color = "红色"
    elif ask_yes_no("那是黄色的吗？"):
        color = "黄色"
    elif ask_yes_no("那是绿色的吗？"):
        color = "绿色"
    else:
        color = "其他颜色"
    ...
    if ask_yes_no("是圆形的吗？"):
        shape = "圆形"
    elif ask_yes_no("那是长条形的吗？"):
        shape = "长条形"
    else:
        shape = "其他形状"

    # 第三步：光凭颜色+形状还分不清——苹果、樱桃、荔枝都是"红色+圆形"，
    # 得再多问一个大小，尽量问细一点才下结论
    size = "大" if ask_yes_no("个头比乒乓球大吗？") else "小"

    key = (color, shape, size)
    total += 1
    if key in TRAINING_DATA:
        print(f"AI猜：{TRAINING_DATA[key]}！（这个颜色+形状+大小的组合我训练时见过）")
        if ask_yes_no("AI猜对了吗？"):
            print("猜对了！")
            correct += 1
        else:
            print("猜错了——这个特征组合虽然AI训练时见过，但撞上了别的水果，光靠这几个特征还是分不清。")
    else:
        print("颜色、形状、大小我都猜了，但这个组合我训练时没见过，猜不出来了……")
        real_name = input("你能直接告诉我这是什么水果吗（说出它的名字）？")
        print(f"哦，原来是「{real_name}」啊！但下次再遇到同样的特征，我还是认不出来——")
        print("因为我并没有真的'学会'它，只是这次你告诉了我答案而已。")
...
print("就算多问几轮、问得再细，AI也只对训练数据里学过的组合有把握。")`,
    hint: `AI会依次猜颜色、形状、大小，猜错了会自动换一个继续问，不用你操心。想让AI"露馅"，就想一个训练数据里没有的组合，比如小个头的樱桃（红色+圆形+小）。`,
    walkthrough: [
      { lines: [1, 3], note: `开头注释点明这一关的重点：AI不能光凭颜色+形状就下结论（苹果、樱桃、荔枝都是红色+圆形），得多问一个特征才行；下了结论之后还要问你对不对。` },
      { lines: [6, 7], note: `小工具函数：把"是/否"的文字回答转换成True/False，方便后面直接用在if判断里。` },
      { lines: [10, 14], note: `训练数据：只学过"红色+圆形+个头大=苹果"和"黄色+长条形+个头大=香蕉"这2种组合——这次是三个特征凑成的key，不是两个了。` },
      { lines: [16, 17], note: `开场白：诚实地告诉你AI只学过2个例子，接下来随便你想什么水果都可以拿来考它。` },
      { lines: [19, 20], note: `记两个数字：一共考了几轮、AI答对了几轮，最后要用来算成绩。` },
      { lines: [22, 22], note: `用一个死循环让游戏可以一直玩下去，退出的时机在循环最后一步判断。` },
      { lines: [23, 23], note: `打印当前是第几轮，方便你知道进度。` },
      { lines: [25, 33], note: `猜颜色：先猜"红色"，你说不是就换猜"黄色"，还不是就换猜"绿色"，都不是就归为"其他颜色"——这就是"猜错了换一个方向继续猜"的具体做法：用elif一级一级往下试。` },
      { lines: [35, 41], note: `猜形状，用一模一样的思路：猜"圆形"，不对就猜"长条形"，都不对就归为"其他形状"。` },
      { lines: [43, 45], note: `光凭颜色+形状还分不清——现实里苹果、樱桃、荔枝都是"红色+圆形"，所以再多问一个大小，尽量把特征问细一点，才不会随便下结论。` },
      { lines: [47, 48], note: `把猜到的颜色、形状、大小拼成一个三元组，当作查训练数据用的key；不管接下来猜不猜得出来，这一轮都要算进"总共考了几次"里。` },
      { lines: [49, 55], note: `如果这三个特征的组合训练时见过，就报出对应的水果——但还是要问你"猜对了吗"，因为就算问了三个特征，也可能撞上别的长得一模一样的水果。` },
      { lines: [56, 60], note: `如果这个组合训练时没见过：AI没有硬编一个答案糊弄你，而是老实承认猜不出来，直接问你这是什么水果——问到答案之后，也诚实说明"这不算真的学会了，只是这次你告诉了我"。` },
      { lines: [62, 64], note: `问你还要不要继续考它，回答"否"（或者其他任何不是"是"的内容）就跳出循环，结束游戏。` },
      { lines: [65, 65], note: `打印一个空行，让下一轮的"第X轮"标题前面留个间隔，不会紧贴着上一轮的内容。` },
      { lines: [67, 69], note: `游戏结束，算出这一局的最终成绩，再次点明规律：不管AI问多少轮问题、问得多细致，它也只对训练数据里学过的组合有把握——这就是为什么不能盲目相信AI的每一个答案，得自己核实。` },
    ],
  },
  {
    id: 6,
