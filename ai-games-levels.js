...
print("真实的AI（比如神经网络）原理类似，只是有几十亿个这样的神经元一起工作。")`,
  ...
  },
  {
    ...
    explain: `
      <p>这个AI"水果分类器"只学过2个例子：红色+圆形=苹果，不红+不圆=香蕉。现在拿6种真实水果
      考考它——你会发现AI犯错至少有两种情况：一种是完全没见过的组合（比如草莓：红色但不是圆形），
      只能瞎猜；另一种更隐蔽——樱桃跟苹果特征"撞车"（都是红色+圆形），AI会很"自信"地把樱桃说成
      苹果，因为它就只看得懂这两个特征，根本分不出樱桃和苹果的区别。</p>
      <p>每按一次回车，看AI猜一种水果，最后会给出它的正确率。</p>
    `,
...
# 拿6种真实水果考考这个AI——AI并不知道这份名单，只是用来最后跟它的猜测对比。
# 注意樱桃、菠萝刻意选了跟训练数据"特征撞车"的水果：樱桃也是红色+圆形，
# 菠萝也是不红+不圆，AI会照抄训练时的答案，但答案其实是错的。
FRUITS_TO_TEST = [
    ("苹果", True, True),
    ("香蕉", False, False),
    ("草莓", True, False),    # 红色，但不是圆形——AI没学过这种组合
    ("西瓜", False, True),    # 不是红色，但是圆形——AI也没学过
    ("樱桃", True, True),     # 特征跟苹果一模一样，AI会误判成苹果
    ("菠萝", False, False),   # 特征跟香蕉一模一样，AI会误判成香蕉
]
...
def ai_guess(is_red, is_round):
    ...
    if key in TRAINING_DATA:
        return TRAINING_DATA[key], True
    # 没见过这种组合——AI只能从学过的两种里瞎猜一个
    return "苹果（瞎猜的）", False


print("这个AI只学过2个例子：红色+圆形=苹果，不红+不圆=香蕉。")
print("现在拿6种真实水果考考它，看它能猜对几个！\\n")

correct = 0
for name, is_red, is_round in FRUITS_TO_TEST:
    input(f"下一个水果是：{name}（按回车看AI怎么猜）")
    guess, seen_before = ai_guess(is_red, is_round)
    print(f"AI看到的特征：红色={is_red}，圆形={is_round}")
    print(f"AI猜：{guess}")
    if name in guess:
        print("AI猜对了！")
        correct += 1
    elif seen_before:
        print(f"AI猜错了，正确答案是：{name}——这次AI其实很'自信'，")
        print(f"因为这个特征组合它训练时见过，但训练时对应的是别的水果，")
        print(f"只看红色+圆形这两个特征，AI根本分不清{name}和{guess}。")
    else:
        print(f"AI猜错了，正确答案是：{name}（它训练时没见过这种特征组合）")
    print()

print(f"最终成绩：AI答对了 {correct}/{len(FRUITS_TO_TEST)} 个。")
print("发现规律了吗？AI犯错至少有两种原因：一种是完全没见过的组合（瞎猜），")
print("另一种是见过这个组合，但只靠这么少的特征，根本分不清两种不同的水果——")
print("这就是为什么用AI的时候，不能盲目相信它的每一个答案，得自己核实。")`,
    hint: `游戏会依次拿6种水果考AI。留意樱桃和菠萝——它们跟训练数据"撞脸"（特征一样），AI会很自信地答错；草莓和西瓜则是AI完全没见过的组合，只能瞎猜。`,
    walkthrough: [
      { lines: [1, 1], note: `开头注释点明这一关要探索的问题：给AI的例子很少的时候，它会怎么样。` },
      { lines: [3, 8], note: `训练数据：字典的key是"(是否红色, 是否圆形)"这样一对布尔值，value是水果名字——注意这里刻意只给了2种组合，很多真实情况根本没覆盖到。` },
      { lines: [10, 20], note: `拿来考AI的6种真实水果，各自的真实特征都写死在这里——这份名单只是给游戏自己核对用的，AI在猜的时候是看不到的。樱桃、菠萝是刻意选的："撞脸"训练数据的水果。` },
      { lines: [23, 28], note: `预测函数：如果这个特征组合在训练数据里见过，就直接查表返回对应答案（第二个值True表示"这个组合它见过"）；没见过就只能从学过的两种里随便挑一个，返回False表示"没见过"。` },
      { lines: [31, 32], note: `开场白：诚实地告诉你这个AI只学过2个例子，接下来要拿真实水果考它。` },
      { lines: [34, 34], note: `记一下AI总共答对了几个，最后要算它的正确率。` },
      { lines: [35, 35], note: `依次拿出每一种水果——同时拿到它的名字和真实特征。` },
      { lines: [36, 36], note: `先揭晓这一轮考的是什么水果，按回车才继续看AI怎么猜——留出停顿，方便你自己也先猜一下AI会不会猜对。` },
      { lines: [37, 37], note: `把这个水果的特征交给AI，同时拿到它的猜测，和"这个组合它是否见过"。` },
      { lines: [38, 39], note: `把AI看到的特征和它的猜测都打印出来。` },
      { lines: [40, 42], note: `猜对了：记一分，打印祝贺。` },
      { lines: [43, 46], note: `猜错了，但这个特征组合AI训练时其实见过——说明AI不是没见过，而是只靠这两个特征，根本区分不了两种长得"特征一样"的水果，答得很自信但是错的。` },
      { lines: [47, 48], note: `猜错了，而且这个特征组合AI压根没见过——这才是最直接的"数据不够"型出错。` },
      { lines: [49, 49], note: `打印一个空行，让每一轮的结果之间留出间隔，看着不挤在一起。` },
      { lines: [51, 54], note: `6种水果都考完了，算出最终正确率，然后总结两种出错的原因：完全没见过的组合只能瞎猜；见过的组合特征太粗糙也会认错——这就是为什么不能盲目相信AI的每一个答案，得自己核实。` },
    ],
  },
  {
    id: 6,
