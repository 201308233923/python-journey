...
print("真实的AI（比如神经网络）原理类似，只是有几十亿个这样的神经元一起工作。")`,
  ...
  },
  {
    ...
    explain: `
      <p>这个AI"水果分类器"只学过2个例子：红色+圆形=苹果，不红+不圆=香蕉。当你问它没见过的组合
      （比如"红色但不是圆形"），它要么答错，要么自己也说不确定——这就是为什么AI训练数据太少或者
      太偏，判断就会不靠谱。</p>
      <p>回答两个"是/否"问题，看看AI能不能猜对你心里想的水果。</p>
    `,
...
def ai_guess(is_red, is_round):
    ...
    # 没见过这种组合——AI只能瞎猜一个最接近的
    return "不确定，AI没学过这种组合，随便猜一个：苹果？", False
...
print("试试描述一个水果，看AI能不能猜对！\\n")

is_red = ask_yes_no("这个水果是红色的吗？")
is_round = ask_yes_no("这个水果是圆形的吗？")

guess, confident = ai_guess(is_red, is_round)

print(f"\\nAI猜：{guess}")
if not confident:
    print("看到了吗？训练数据太少，AI一遇到没见过的组合就抓瞎——")
    print("比如'红色但不是圆形'（草莓、红辣椒都是），这个AI根本没学过。")
    print("真实的AI也是一样：训练数据覆盖不到的情况，它照样会出错，")
    print("这就是为什么用AI的时候，不能盲目相信它的每一个答案，得自己核实。")`,
    hint: `先随便选两个"是/否"的组合试试；重点看当你选的组合AI没学过时（比如红色但不是圆形），它会怎么回答。`,
    walkthrough: [
      ...
      { lines: [11, 16], note: `预测函数：如果这个特征组合在训练数据里见过，就直接查表返回对应答案（第二个值True表示"有把握"）；没见过就只能瞎猜一个，返回False表示"没把握"。` },
      { lines: [19, 21], note: `一个小工具函数：把"是/否"的文字回答转换成True/False，方便后面直接用布尔值判断。` },
      { lines: [24, 25], note: `开场白：诚实地告诉你这个AI只学过2个例子——先立好"它其实很有限"这个预期。` },
      { lines: [27, 28], note: `问你两个问题，把回答转成布尔值，拼成AI需要的那个特征组合。` },
      { lines: [30, 30], note: `把你的答案交给AI，同时拿到"猜的是什么"和"有没有把握"两个结果。` },
      { lines: [32, 32], note: `把AI的猜测打印出来，不管有没有把握都会给一个答案——这一点本身也值得注意：AI很少会主动说"我不知道"。` },
      { lines: [33, 37], note: `如果AI其实没见过这种组合（没把握），揭晓真相：训练数据太少的时候，AI一遇到没学过的情况就可能瞎蒙——这就是为什么用AI给的答案，不能不假思索地全部相信，得自己核实一下。` },
    ],
  },
  {
    id: 6,
