...
if name == "XOR":
  ...
  },
  {
    ...
    explain: `
      ...
      问遍了所有特征还是筛不出唯一答案，AI会直接问你真正的水果是什么，然后把它记进自己认识的
      名单——哪怕这次没来得及问完全部6个特征也没关系，没问到的地方就先当"不确定，暂时不影响
      判断"，下次再遇到类似的就更容易猜对，这就是"从错误里学习"。</p>
    `,
...
print("发现错误、告诉它正确答案，它才能跟着学到新东西，这也是真实AI改进的方式之一。")`,
    hint: `想让AI很快猜中，就想一个特征很鲜明的水果（比如香蕉）；想考验AI，就想樱桃或者荔枝——它俩长得很像，AI得靠"表皮有没有刺"才能分清。想看AI"学新水果"，就想一个不在9种里的水果（比如火龙果），猜错/猜不出来之后告诉它名字，再考一次同样的水果试试，看它是不是更容易猜中了。`,
    walkthrough: [
      { lines: [1, 3], note: `开头注释点明这一关的核心思路："排除法"——每问一个特征就排除掉一批不符合的候选，问到只剩一个就是答案；第3行是新加的一句，提前预告"猜错/猜不出来会把新水果学进去"这件事。` },
      { lines: [5, 15], note: `AI"认识"的水果，一开始有9种，每种都标了6个特征——这就是AI最初的全部知识，特征标得越准，猜得就越准；游戏过程中学到的新水果，也会追加进这份名单，只是可能没有全部6个特征。` },
      ...
      { lines: [32, 41], note: `新增的函数：猜错/猜不出来之后，直接问真正的水果叫什么——如果这个名字AI其实已经认识（比如特征刚好跟另一种水果撞车了），就提示一下，不重复添加；如果确实是没见过的新水果，就把它连同这一轮已经问到的特征（不一定是全部6个）一起追加进FRUITS。` },
      { lines: [44, 46], note: `开场白：告诉你AI一开始认识哪9种水果，第3句提前说明"猜错/猜不出来会学新水果"这个规则。` },
      { lines: [48, 49], note: `记两个数字：一共考了几轮、AI答对了几轮，最后要用来算成绩。` },
      { lines: [51, 54], note: `用一个死循环让游戏可以一直玩下去；候选名单candidates每一轮都从当前的FRUITS重新复制（如果上一轮学了新水果，这里也会包含进来）；answers是记这一轮已经问到的每个特征答案的字典。` },
      { lines: [56, 64], note: `排除法的核心：依次问每个特征，把答案存进answers；用 .get(feature, answer) 而不是直接按key取值——如果某个候选水果（比如中途学到的新水果）还没被问到过这个特征，就用当前的answer当默认值，相当于"这一项还不确定，先当它符合"，不会因为数据不全就被误判排除掉。` },
      { lines: [66, 72], note: `不管这一轮最后猜不猜得出来都要计数；候选恰好剩1个就是AI的最终猜测，还是要问你"猜对了吗"确认。` },
      { lines: [73, 75], note: `猜错了：直接调用"学新水果"的函数——不需要再补问剩下的特征，哪怕这一轮只问到了一两个特征，也照样能把这个新水果记下来。` },
      { lines: [76, 79], note: `候选剩好几个或者一个都不剩：这时候6个特征其实已经全问完了，同样调用"学新水果"函数——原来这里只是问一下名字然后揭秘，现在会真的把这个新水果记进FRUITS。` },
      { lines: [81, 84], note: `问你还要不要继续考它，回答"否"（或者其他任何不是"是"的内容）就跳出循环，结束游戏。` },
      { lines: [86, 89], note: `游戏结束，算出这一局的最终成绩，新增一句"现在认识的水果一共有几种了"——如果中途学到过新水果，这个数字会比一开始的9大，直观看到AI在变聪明；最后点明：发现错误、告诉它正确答案，它才能跟着学到新东西，这也是真实AI改进的方式之一。` },
    ],
  },
  {
    id: 6,
