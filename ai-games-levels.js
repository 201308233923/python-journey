...
while True:
  ...
  },
  {
    ...
    explain: `
      ...
      <p>为了让它在简单场景下真的能正常聊（比如问它叫什么、说声谢谢），这次给常见话题多配了
      几个不同的问法（"名字"/"你是谁"/"叫什么"都能问到它的身份），抓名字的逻辑也加了防呆——
      "我是谁"、"我是不是走错片场了"这种话不会被误当成在报名字。</p>
      <p>一句话完全没命中关键词时，它这次不会直接干瞪眼——借的是1966年真实存在过的
      ELIZA聊天机器人的老套路：抓住这句话的结构，把"我/你"这类人称对调，拼回一句新的
      问句反问回去（比如"我讨厌下雨"会被接成"是什么让你讨厌下雨？"）。当年很多人真的以为
      ELIZA听懂了自己，其实它一样没有理解语言，纯粹是格式变换——跟RULES的关键词匹配是
      同一件事的另一种做法，只是看起来更像在接话。</p>
      <p>连续两次它都没听懂（触发了兜底回复），它会主动请你教它一条新规则——
      告诉它关键词和你想要的回复，它当场就学会了。这不是真的"理解"，只是多记了一条规则。</p>
      <p>它还会试着记住你的名字——说一句"我叫XX"或者"我是XX"，它会把这个名字存进一个
...
# 常见问法配同一组回复，避免同样的意思因为措辞不同就答不上来
IDENTITY_REPLIES = ["我是你自己写的AI聊天机器人，还没有名字，你来给我起一个？"]
GREETING_REPLIES = ["你好呀！", "嗨，很高兴见到你！"]

# 规则表：关键词 -> 可能的回复（可以自己加更多规则！）
# 注意顺序：像"你好吗"这种"短关键词的一部分"，必须写在"你好"前面——
# 逐个检查是按字典的顺序来的，"你好"排前面的话，"你好吗"永远会先被"你好"截胡，
# 自己那条规则变成摆设，见都见不到。
RULES = {
    "你好吗": ["我很好呀，谢谢关心！", "还不错，你呢？"],
    "你好": GREETING_REPLIES,
    "在吗": GREETING_REPLIES,
    "名字": IDENTITY_REPLIES,
    "你是谁": IDENTITY_REPLIES,
    "叫什么": IDENTITY_REPLIES,
    "谢谢": ["不客气！", "不用谢～"],
...
# "我是"后面接这些词，大概率不是在报名字（比如"我是不是..."、"我是说..."），
# 抓到就当没抓到，不然会把这类话误当成在自我介绍。
NON_NAME_STARTERS = ("不", "说", "想", "要", "在", "真", "觉得", "应该", "可以", "喜欢", "讨厌", "希望", "记得", "认为")
QUESTION_WORDS = {"谁", "什么", "啥", "哪个", "哪儿", "哪里", "哪位"}

# 下面这套借的是1966年ELIZA聊天机器人的老思路：不是每句话都提前配好关键词，
# 而是抓住句子结构、把"我"和"你"互相对调，拼回一句新的问句反问回去——
# 这样即使完全没配过这句原话，也能接得住，聊起来更不容易卡壳。
PRONOUN_SWAPS = [
    ("我们", "你们"), ("你们", "我们"),
    ("我的", "你的"), ("你的", "我的"),
    ("我", "你"), ("你", "我"),
...
def reflect(fragment):
    """把一段话里的"我/你"互相对调；用占位符分两步换，不然"我"换成"你"之后
    又被后面"你换我"的规则再换回去，两条规则会互相打架。"""
    placeholders = [f"\\x00{i}\\x00" for i in range(len(PRONOUN_SWAPS))]
    for i, (old, _) in enumerate(PRONOUN_SWAPS):
        fragment = fragment.replace(old, placeholders[i])
    for i, (_, new) in enumerate(PRONOUN_SWAPS):
        fragment = fragment.replace(placeholders[i], new)
    return fragment.strip("，。！？~～ ")


EMOTION_WORDS = ("难过", "伤心", "开心", "高兴", "生气", "担心", "紧张", "害怕", "累", "无聊", "激动")

# 越具体的规则要排在越前面——这条经验前面在RULES那边已经踩过一次坑了：
# 这里"我(.+)"这条兜底大网必须放在最后，不然前面所有更精确的规则全都会被它
# 提前截胡，一次都轮不到。
ELIZA_PATTERNS = [
    (re.compile(r"我不喜欢(.+)"), ["为什么你不喜欢{}？", "{}是哪里让你不太舒服？"]),
    (re.compile(r"我(?:很|非常|真的|有点|有些)?喜欢(.+)"), ["为什么你喜欢{}？", "{}有什么特别吸引你的地方？"]),
    (re.compile(r"我(?:很|非常|真的|有点|有些)?讨厌(.+)"), ["是什么让你讨厌{}？"]),
    (re.compile(r"我.{0,4}(" + "|".join(EMOTION_WORDS) + ")"), ["为什么你会{}？", "多久有这种感觉了？"]),
    (re.compile(r"我(?:觉得|感觉)(.+)"), ["为什么你觉得{}？", "这种感觉是什么时候开始的？"]),
    (re.compile(r"我想要?(.+)"), ["为什么你想要{}？", "如果真的有了{}，会有什么不一样？"]),
    (re.compile(r"因为(.+)"), ["这就是全部原因吗？", "还有别的原因吗？"]),
    (re.compile(r"为什么(.+)"), ["你自己觉得原因是什么？"]),
    (re.compile(r"(.+)吗[？?]?$"), ["你自己是怎么想的？", "这个对你来说重要吗？"]),
    (re.compile(r"我(.+)"), ["能多说说{}这件事吗？", "为什么会这样？"]),
]


def eliza_reply(user_input):
    """RULES里的关键词一个都没命中时，试试按句子结构反问回去——
    这不是真的"理解"，纯粹是抓一段话、把人称代词对调后拼回新句子。"""
    for pattern, templates in ELIZA_PATTERNS:
        ...
        if "{}" not in template:
            return template
        captured = reflect(match.group(1))
        if captured:
...
def detect_name(user_input):
    ...
    if not (0 < len(name) <= 6):
        return None
    if name in QUESTION_WORDS or name.startswith(NON_NAME_STARTERS):
        return None
    return name
...
def chat():
    ...
    print("句子没命中任何关键词时，它还会试着按句子结构反问回去；连续两次都接不住，")
    print("它会主动请你教一条新规则。输入'再见'结束聊天。\\n")
    miss_streak = 0
    taught_count = 0
    user_name = None
    ...
    print("提示：机器人根本不'理解'你说的话。命中关键词的部分是在查表；没命中时的")
    print("反问，也只是把你的话拆开、换个人称拼回去——本质上还是格式变换，不是真懂。")


chat()`,
    hint: `试试问它"你叫什么"、"你是谁"，或者直接说声"谢谢"、"你好吗"，看它是不是都能答上来；再说一句"我叫小明"，看它记不记得住；也可以试试"我是谁"这种话，看它是不是不会误当成在报名字了。也可以说一句完全没配过关键词的话，比如"我讨厌下雨天"或者"我很难过"，看它是不是能接得住，而不是干瞪眼回一句"嗯嗯，然后呢？"。`,
    walkthrough: [
      { lines: [1, 2], note: `待会儿要从"多个可能的回复"里随机选一句要用random；从"我叫XX"这种话里抓名字要用到正则表达式，得用re模块。` },
      { lines: [4, 6], note: `新增：把"问身份"和"打招呼"各自的回复列表提前定义成变量——同一个意思的好几种问法（比如"名字"/"你是谁"/"叫什么"）可以共用同一份回复，不用在RULES里重复写好几遍一模一样的内容。` },
      { lines: [8, 23], note: `机器人的"知识库"扩充了：同一个话题配了好几个不同的问法，比如问身份现在"名字"、"你是谁"、"叫什么"都能命中；新增了"谢谢"和"你好吗"这两个日常寒暄。注意"你好吗"特意排在"你好"前面——逐个检查是按字典顺序来的，"你好"是"你好吗"的一部分，谁排在前面谁就会先被命中，所以更具体的关键词要写在前面，不然后面那条永远轮不到。` },
      { lines: [25, 29], note: `如果你说的话一个关键词都没命中，就从这里随便回你一句，不会卡住答不上来——待会儿也会用这份列表判断"这次是不是没听懂"。` },
      { lines: [31, 34], note: `新增的两份"防呆"名单：NON_NAME_STARTERS是"我是"后面常见的、明显不是在报名字的开头字（比如"我是不是……"、"我是说……"）；QUESTION_WORDS是"我是谁"这种疑问代词——待会儿抓名字时用来排除这些情况，不然会把这类话误当成在自我介绍。` },
      { lines: [36, 43], note: `新增：反问要用的"人称对调表"——把"我们/你们"、"我的/你的"、"我/你"这几组词两两配对，待会儿reflect()函数会照着这张表把你的话里的人称全部换成对方视角。` },
      { lines: [46, 54], note: `真正做对调的函数：如果直接"我换成你"再"你换成我"这样接连替换，"我"会先变成"你"，紧接着又被第二条规则错误地换回"我"——所以先把每一组的旧词换成一个不会跟中文重复的占位符，两组都替换完占位符之后，再统一把占位符换成新词，就不会互相打架了。` },
      { lines: [57, 73], note: `新增：情绪词列表，加上一份"反问规则表"——按"我不喜欢XX"、"我喜欢XX"、说到情绪词、"我觉得XX"、"我想要XX"、"因为XX"、"为什么XX"、"XX吗"，一直到完全兜底的"我XX"，从具体到笼统排好顺序。最后这条"我(.+)"必须放在最后——道理跟前面RULES的"你好吗"要排在"你好"前面完全一样：越笼统的规则越要往后放，不然会把后面更精确的规则全部截胡。` },
      { lines: [76, 89], note: `RULES一个关键词都没命中时，就按顺序试这些反问规则：抓到匹配就从对应的模板里随机挑一句，模板里带{}的话，就把抓到的这一小段先做人称对调，再填进去拼成新句子；如果这批规则一条都没接住，就返回None，交给下一层兜底。` },
      { lines: [92, 99], note: `完整的回复流程变成三层：先查RULES关键词表，查不到就试着按句子结构反问，还是接不住才随机挑一句"嗯嗯，然后呢？"这类真正的兜底回复——is_default那句判断连续没听懂/要不要触发教学，靠的正是"是不是落到了这最后一层"。` },
      { lines: [102, 113], note: `抓名字的函数：先用正则找"我叫XX"或"我是XX"，抓出后面一小段——这次多了两道防呆检查：抓到的词如果是"谁"这类疑问代词，或者整句话是以"不"、"说"这些明显不是在报名字的字开头，就当作没抓到，返回None，不会误判。` },
      { lines: [116, 128], note: `教学逻辑不变：先问关键词，再问希望听到这个关键词时怎么回复——两个都填了才真正写进RULES字典，返回True告诉外面"这次真的学会了一条"。` },
      { lines: [131, 137], note: `定义整个对话流程，开场白提到"会反问"和"连续两次没听懂会请教"这两个行为；user_name专门用来存你的名字，一开始是None（还不知道）。` },
      { lines: [139, 146], note: `每一轮先试着从你的话里抓名字——如果抓到了（通过了防呆检查）、而且之前还不知道你的名字，就记下来并回一句确认，然后直接跳到下一轮。` },
      { lines: [148, 153], note: `正常的回复流程：挑一句回复（关键词/反问/兜底三层里的一种）；如果已经知道你的名字、而且这次是落到兜底层的默认回复，就在前面加上称呼；打印出来，说"再见"就结束对话。` },
      { lines: [155, 158], note: `判断刚才这句是不是落到了最后的兜底默认回复——如果是，说明关键词和反问规则都没接住，连续没接住的计数+1；只要有一层接住了就清零，重新计数。` },
      { lines: [160, 163], note: `连续没接住够2次，就触发"教学"环节；真的教成功了才把taught_count加1，不管教没教成功都把计数清零，避免刚教完马上又立刻重复弹出来。` },
      { lines: [165, 170], note: `对话结束，报告教会了几条规则；如果记住了名字，额外点明这是"真正存进变量的记忆"，跟"关键词匹配/反问"都不是一回事——一个是临时查表或格式变换，一个是一直带着用的状态。` },
      { lines: [173, 173], note: `真正开始运行整个对话，前面都只是定义。` },
    ],
  },
  {
    id: 3,
...
print("__GAME_OUTCOME__:WIN" if you_score >= ai_score else "__GAME_OUTCOME__:LOSE")`,
  ...
  },
  {
    ...
    explain: `
      ...
      解决单层做不到的问题的。</p>
      <p>训练完一种逻辑之后，可以换另一种逻辑接着训练，不用重开一局——建议依次试完
      AND、OR、XOR三种，直接对比"单个神经元哪些逻辑能学会、哪些学不会"，这个对比比
      只看一种更能看出问题出在哪。</p>
    `,
    code: `import random
import math
...
def choose_and_train():
    ...
    return two_layer_correct


last_result = choose_and_train()

while True:
    again = input("\\n要不要换一种逻辑再训练一次？比如AND/OR都学会了，可以换XOR感受一下神经元的极限（输入 是/否）：").strip()
    if again != "是":
        break
    last_result = choose_and_train()

print("__GAME_OUTCOME__:WIN" if last_result else "__GAME_OUTCOME__:LOSE")`,
    hint: `输入 1 训练AND逻辑，输入 2 训练OR逻辑。想看AI的极限在哪，输入 3 训练XOR——练多少轮都学不会才是正常的。选完XOR之后，一定要试试"要不要看两层神经元"那个选项，输入"是"，看单层学不会的问题，加一层之后是怎么被解决的。训练完一种逻辑，别急着结束，选"要不要换一种逻辑再训练一次"依次把AND、OR、XOR都试一遍，直接对比"能学会"和"学不会"这两种结果。`,
    walkthrough: [
      ...
      { lines: [75, 76], note: `sigmoid把任意一个数字压缩到0~1之间——两层网络的每个神经元都要经过这一步再往下传，这是让"多层叠加"真正有意义的关键（如果没有这一步，无论叠多少层，数学上都等价于一层，一样学不会XOR）。` },
      { lines: [79, 102], note: `两层网络训练函数：2个输入先经过3个隐藏神经元（每个都用sigmoid处理），再汇总到1个输出神经元；每条数据算完预测，把误差用"反向传播"这个方法，从输出神经元往回一层层传给隐藏神经元，各自按自己对误差的"贡献大小"微调权重——这比单神经元的调整规则复杂，但思路是相通的：错了就往对的方向挪一点。` },
      ...
      { lines: [126, 133], note: `新增：把"选逻辑、训练、测试"包成一个函数，方便待会儿反复调用——不用重开一局就能连续训练好几种逻辑，直接对比结果。` },
      { lines: [135, 139], note: `如果这次训练的不是XOR（也就是AND或OR）：直接总结"随机起步、猜错就微调"这个学习的本质，把这次是不是全对的结果交出去，函数到此结束。` },
      { lines: [141, 144], note: `训练的是XOR：不管练得怎么样都直接揭示真相——AND/OR能画一条直线分开两类点，XOR不管怎么画都分不开，这是单个神经元的数学天花板。` },
      { lines: [146, 148], note: `问你要不要亲眼看两层网络解决XOR——选"否"就直接把单神经元这次的结果交出去，不往下走升级流程。` },
      { lines: [150, 159], note: `选"是"：真的训练一个两层网络并测试，根据这次是不是真的全部学会了，给出对应的解读（学会了就点出"深度"的含义；没学会就如实说明训练本身有随机成分，这也是真实场景会遇到的情况），把两层网络这次的结果交出去。` },
      { lines: [162, 162], note: `真正开始：先选一次逻辑，跑一次完整的训练+测试，记住这次的结果。` },
      { lines: [164, 168], note: `新增：问要不要换一种逻辑再训练一次——不是"是"就结束；是的话再跑一次choose_and_train()，用这次新的结果覆盖掉上一次记的结果，可以这样一直换着训练AND/OR/XOR，直接对比谁能学会谁学不会。` },
      { lines: [170, 170], note: `游戏结束时打印结算暗号——依据的是"最后一次训练"的结果（如果最后练的是XOR还选了升级，就看两层网络学会了没有），页面看到"全对"才会放庆祝的礼花。` },
    ],
  },
  {
    id: 5,
...
print("发现错误、告诉它正确答案，它才能跟着学到新东西，这也是真实AI改进的方式之一。")`,
  ...
  },
  {
    ...
    explain: `
      ...
      （比如同时提到"恐龙"和"化石"），它越能把最贴切的那本排到最前面。这次AI打分不是简单数
      "命中了几个词"，而是按每个词的"稀有程度"加权：像"冒险"这种好几本书都有的词，命中一次分量不大；
      像"宇航员"这种只有1本书才有的词，命中一次分量就重得多——这就是真实搜索引擎/推荐系统排序背后
      常用的思路（关键词越少见、越能"一锤定音"，权重就越高）。可以连着搜好几次，对比换个说法之后
      推荐结果怎么变。</p>
      <p>搜完之后可以选择反过来玩：这次AI心里想一本书，一个个透露它的标签，标签越少就能锁定
      答案，说明你越会抓"区分度高"的关键词——跟前面"搜书"是同一个道理反过来体验一遍。</p>
    `,
    code: `import random

# AI图书管理员：练习怎么问AI才能得到有用的答案
...
BOOKS = [
    ...
    {"title": "《名侦探的秘密案件》", "tags": ["推理", "侦探", "案件", "解谜"]},
    {"title": "《恐龙与化石猎人》", "tags": ["恐龙", "化石", "冒险", "科普"]},
    {"title": "《魔法森林的秘密》", "tags": ["魔法", "奇幻", "森林", "悬疑"]},
]


def tag_rarity_scores():
    """算出每个标签在书单里有多"稀有"——出现的书越少，说明这个词区分度越高，
    命中一次的价值也越大；很多本书都有的词（比如"冒险"），单独命中一次分量
    就没那么重。"""
    counts = {}
    for book in BOOKS:
        for tag in book["tags"]:
            counts[tag] = counts.get(tag, 0) + 1
    return {tag: round(len(BOOKS) / count, 2) for tag, count in counts.items()}


TAG_SCORES = tag_rarity_scores()
...
def search_books(query):
    ...
    for book in BOOKS:
        ...
        if hits:
            score = round(sum(TAG_SCORES[tag] for tag in hits), 2)
            scored.append((book["title"], score, hits))
...
def search_once():
    ...
    if results:
        print("\\nAI推荐（按关键词'区分度'加权算分，从高到低排序）：")
        for title, score, hits in results:
            print(f"  {title} —— 命中：{'、'.join(hits)}，区分度打分：{score}")
        print("\\n分数不是简单数'命中了几个词'：命中一个很多本书都有的词（比如'冒险'，4本书都有），")
        print("加的分不多；命中一个很少书才有的词（比如'宇航员'，只有1本书有），加的分反而更多——")
        print("这就是为什么问题越具体，AI给出的排序越有区分度，而不是一堆书打平分。")
    else:
...
def reverse_guess_challenge():
    """反过来：AI心里想一本书，一个个透露标签，看你要多少个标签才能猜中"""
    secret = random.choice(BOOKS)
    remaining_tags = secret["tags"][:]
    random.shuffle(remaining_tags)
    revealed = []

    print("\\n换个玩法：这次AI心里想着一本书，会一个一个透露它的标签——")
    print("标签越少就能猜中，说明你越会抓关键信息；标签透露完了还没猜中，AI会直接揭晓答案。\\n")

    while remaining_tags:
        tag = remaining_tags.pop(0)
        revealed.append(tag)
        matches = [book["title"] for book in BOOKS if all(t in book["tags"] for t in revealed)]
        print(f"透露一个新标签：{tag}（目前已知标签：{'、'.join(revealed)}）")
        if len(matches) == 1:
            print("这些标签已经能唯一确定一本书了，要不要猜猜看？")

        guess = input("直接说出书名来猜（还没想好，回车听下一个标签）：").strip()
        if not guess:
            continue
        if guess == secret["title"]:
            print(f"\\n猜对了！只用了 {len(revealed)} 个标签就找到了{secret['title']}。")
            return len(revealed)
        print(f"不对，不是{guess}——再听一个标签试试。")

    print(f"\\n标签全透露完了，答案是{secret['title']}。")
    return len(revealed)
...
search_once()
while True:
    again = input("\\n要不要换个问法，再搜一次看看推荐结果会怎么变？（输入 是/否）：").strip()
    if again != "是":
        break
    search_once()

want_reverse = input("\\n要不要反过来试试，这次换AI出题、你来猜书名？（输入 是/否）：").strip()
if want_reverse == "是":
    tags_used = reverse_guess_challenge()
    print("\\n发现了吗？标签（关键词）越少就能锁定答案，说明这些词区分度越高——")
    print("这跟前面'搜书'是同一个道理反过来看：问AI问题时，挑区分度高的关键词，比堆一大堆词更有用。")
    print("__GAME_OUTCOME__:WIN" if tags_used <= 2 else "__GAME_OUTCOME__:LOSE")`,
    hint: `先试试只说一个很多本书都有的模糊词（比如"冒险"），看看是不是好几本书打平分；再试试加一个更少见的词（比如"恐龙 冒险"），对比一下哪本书的分数明显跳到最前面。玩反过来那个模式时，AI报出第一个标签就试着猜猜看——越早猜中，说明你对"这个特征通常对应什么"的判断越准。`,
    walkthrough: [
      { lines: [1, 1], note: `反过来轮到AI出题的环节要随机选书、打乱标签透露顺序，需要用到这个内置模块。` },
      { lines: [3, 3], note: `开头注释点明这一关要练的是"怎么问"，不是"AI怎么答"——AI能不能帮上忙，很大程度上取决于你问得够不够具体。` },
      { lines: [5, 15], note: `书单9本，好几本书故意共用了一些标签（比如"冒险"、"科普"、"恐龙"都在不止一本书里出现），这样同一句话可能同时命中好几本，才需要排序逻辑决定"谁更贴切"——每本书的标签组合都是唯一的，不会有两本书标签完全一样、永远分不清。` },
      { lines: [18, 26], note: `新增：算一下每个标签在9本书里一共出现了几次，再用"书的总数÷出现次数"算出一个分数——出现次数越少（越稀有），分数越高。比如只在1本书出现的标签能拿到满分9.0，4本书都有的"冒险"只能拿到2.25。这跟真实搜索引擎/推荐系统给关键词算权重（专业说法叫TF-IDF）是同一个思路。` },
      { lines: [29, 29], note: `提前把每个标签的稀有度分数算好存起来，待会儿搜索的时候直接查表用，不用每次搜索都重新数一遍。` },
      { lines: [32, 40], note: `搜索函数升级了：不再是简单数"命中了几个标签"，而是把命中的每个标签的稀有度分数加起来当作这本书的总分——命中一个稀有词比命中好几个大众词更能拉开分差，再按总分从高到低排序。` },
      { lines: [43, 58], note: `新增：把"问一次、看一次结果"包成一个函数，方便待会儿反复调用。有结果的话，把命中的标签和这本书的总分都亮出来，并解释一下分数是怎么算的；一本都没搜到，就给出具体的改进建议。` },
      { lines: [61, 66], note: `反向挑战函数：从书单里随机选一本当"秘密书"，把它的标签复制一份打乱顺序（避免每次都按书单里写的固定顺序透露），revealed用来记已经透露过的标签。` },
      { lines: [68, 69], note: `开场白：说明玩法——标签透露得越少就猜中，说明你越会抓关键信息。` },
      { lines: [71, 77], note: `每次从打乱的顺序里取一个新标签透露出来；同时算一下"目前书单里，标签全部符合已知条件的还剩几本"——如果刚好剩1本，提示你可以试着猜了。` },
      { lines: [79, 85], note: `读你的猜测——直接回车（不猜）就继续听下一个标签；猜对了把用了几个标签交出去；猜错了提示继续。` },
      { lines: [87, 88], note: `标签全透露完了你还没猜中，AI直接公布答案，同样把用了几个标签（也就是全部标签数）交出去。` },
      { lines: [91, 92], note: `开场白直接提示了这一关的技巧：关键词越具体，AI才越容易帮到你。` },
      { lines: [94, 99], note: `新增：先搜一次，然后问要不要换个问法再搜——可以反复搜好几次，直接对比换了说法之后AI给出的打分和排序有什么变化，而不是只能搜一次就结束。` },
      { lines: [101, 106], note: `问你要不要反过来玩，玩了就跑刚才那个挑战函数，根据用了几个标签给出总结（区分度越高的词，越少就能锁定答案），打印结算暗号——用的标签数不超过2个才算"赢"，页面看到这个暗号才会放庆祝的礼花。` },
    ],
  },
  {
    id: 7,
