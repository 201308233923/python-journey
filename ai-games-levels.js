...
print("真实的AI（比如神经网络）原理类似，只是有几十亿个这样的神经元一起工作。")`,
  ...
  },
  {
    ...
    explain: `
      <p>这个AI"水果分类器"只学过2个例子：红色+圆形=苹果，不红+不圆=香蕉。现在拿4种真实水果
      考考它——遇到训练时学过的组合，它能答对；遇到没学过的组合（比如草莓：红色但不是圆形），
      它就会瞎猜出错。看它最后能拿多少分，就知道"训练数据不够"会造成多大的影响。</p>
...
# 拿4种真实水果考考这个AI——AI并不知道这份名单，只是用来最后跟它的猜测对比
FRUITS_TO_TEST = [
    ("苹果", True, True),
    ("香蕉", False, False),
...
print("发现规律了吗？AI只对训练数据里出现过的组合有把握，")
print("像草莓（红色但不圆）、西瓜（圆但不红）这种没学过的组合，它就抓瞎——")
print("这就是为什么用AI的时候，不能盲目相信它的每一个答案，得自己核实。")`,
    hint: `游戏会依次拿4种水果考AI，每次按回车看它猜一个。留意后两种（草莓、西瓜）——它们的特征组合AI训练时没见过，AI大概率会猜错。`,
    walkthrough: [
      ...
      { lines: [10, 16], note: `拿来考AI的4种真实水果，各自的真实特征都写死在这里——这份名单只是给游戏自己核对用的，AI在猜的时候是看不到的。` },
      { lines: [19, 24], note: `预测函数：如果这个特征组合在训练数据里见过，就直接查表返回对应答案；没见过就只能从学过的两种里随便挑一个，返回"苹果（瞎猜的）"。` },
      { lines: [27, 28], note: `开场白：诚实地告诉你这个AI只学过2个例子，接下来要拿真实水果考它。` },
      { lines: [30, 30], note: `记一下AI总共答对了几个，最后要算它的正确率。` },
      { lines: [31, 31], note: `依次拿出每一种水果——同时拿到它的名字和真实特征。` },
      { lines: [32, 32], note: `先揭晓这一轮考的是什么水果，按回车才继续看AI怎么猜——留出停顿，方便你自己也先猜一下AI会不会猜对。` },
      { lines: [33, 33], note: `把这个水果的特征交给AI，拿到它的猜测。` },
      { lines: [34, 35], note: `把AI看到的特征和它的猜测都打印出来。` },
      { lines: [36, 40], note: `拿AI的猜测跟真实水果名对比：猜对了记一分；猜错了告诉你正确答案，并且点明原因——这个特征组合它训练时没见过。` },
      { lines: [41, 41], note: `打印一个空行，让每一轮的结果之间留出间隔，看着不挤在一起。` },
      { lines: [43, 46], note: `4种水果都考完了，算出最终正确率，然后揭示规律：AI只对训练数据里见过的组合有把握，没见过的组合就会出错——这就是为什么不能盲目相信AI的每一个答案，得自己核实。` },
    ],
  },
  {
    id: 6,
