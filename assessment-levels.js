...
print(counts)`,
    ...
    check: (r) => {
      ...
      // 出现过（比如苹果2次香蕉3次，数字对调）也会通过；漏掉"橙子"不管（比如
      // if/elif漏写了一支）也只会让字典里少一个键，前两个词的次数依然对得上，
      // 光看苹果+香蕉两项测不出来，所以橙子也必须一起验证。这一关允许打印
      // 字典或者分别打印，格式不固定，所以用"词后面隔几个字符内出现对应
      // 次数"这种宽松的邻近匹配，而不是要求完全固定的格式。
      const okApple = /苹果.{0,8}3/.test(r.stdout);
      const okBanana = /香蕉.{0,8}2/.test(r.stdout);
      const okOrange = /橙子.{0,8}1/.test(r.stdout);
      if (okApple && okBanana && okOrange) {
        return { pass: true, message: "词频统计正确：苹果3次，香蕉2次，橙子1次。" };
      }
      return { pass: false, message: "统计结果不太对：苹果应该出现3次，香蕉2次，橙子1次，检查一下计数逻辑（是不是漏掉了某个词，或者哪个词的次数算错了）。", reviewLevel: 10 };
    },
  },
  {
    id: 6,
